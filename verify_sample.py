...
def main():
    ...
    accuracy_first_pass = round((correct_count + partial_count * 0.5) / len(VERDICTS) * 100, 1)
    accuracy_after_correction = round(correct_signals / total_signals * 100, 1)
    accuracy_apps_after = round(correct_count / len(VERDICTS) * 100, 1)

    out = {
        "sample_size": len(VERDICTS),
        "total_signals_checked": total_signals,
        "first_pass": {
            "apps_correct": correct_count,
            "apps_partial": partial_count,
            "apps_wrong": wrong_count,
        ...
        "after_correction": {
            "signals_correct": correct_signals,
    ...
    print(f"First-pass app accuracy     : {correct_count}/{len(VERDICTS)} = {round(correct_count/len(VERDICTS)*100,1)}%")
    print(f"Apps with any partial/wrong : {partial_count + wrong_count}")
    print(f"Corrections applied         : {out['after_correction']['corrections_applied']}")
    print(f"Signal-level accuracy (post): {correct_signals}/{total_signals} = {accuracy_after_correction}%")
    print(f"Lift                        : +{out['lift_pct_points']} percentage points")
    print()
    print("Corrections:")
    for v in verdicts:
